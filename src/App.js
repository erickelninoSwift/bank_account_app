import "./App.css";
import React, { useReducer } from "react";
/*
INSTRUCTIONS / CONSIDERATIONS:

1. Let's implement a simple bank account! It's similar to the example that I used as an analogy to explain how useReducer works, but it's simplified (we're not using account numbers here)

2. Use a reducer to model the following state transitions: openAccount, deposit, withdraw, requestLoan, payLoan, closeAccount. Use the `initialState` below to get started.

3. All operations (expect for opening account) can only be performed if isActive is true. If it's not, just return the original state object. You can check this right at the beginning of the reducer

4. When the account is opened, isActive is set to true. There is also a minimum deposit amount of 500 to open an account (which means that the balance will start at 500)

5. Customer can only request a loan if there is no loan yet. If that condition is met, the requested amount will be registered in the 'loan' state, and it will be added to the balance. If the condition is not met, just return the current state

6. When the customer pays the loan, the opposite happens: the money is taken from the balance, and the 'loan' will get back to 0. This can lead to negative balances, but that's no problem, because the customer can't close their account now (see next point)

7. Customer can only close an account if there is no loan, AND if the balance is zero. If this condition is not met, just return the state. If the condition is met, the account is deactivated and all money is withdrawn. The account basically gets back to the initial state
*/

const initialState = {
  balance: 0,
  loan: 0,
  isActive: false,
  deposit: 150,
  withdraw: 50,
  loanRequest: 5000,
  payLoan: true,
  closeAccount: false,
};

const reduce = (currentState, action) => {
  if (action.type === "open") {
    return { ...currentState, isActive: action.payload, balance: 500 };
  }
  if (action.type === "deposit") {
    return { ...currentState, balance: currentState.balance + 150 };
  }
  if (action.type === "loan") {
    return {
      ...currentState,
      balance:
        currentState.loan > 0
          ? currentState.balance
          : currentState.balance + currentState.loanRequest,
      loan: currentState.loanRequest,
    };
  }

  if (action.type === "payback") {
    return {
      ...currentState,
      loan: currentState.loan !== 0 ? 0 : currentState.loan,
      balance:
        currentState.loan !== 0
          ? currentState.balance - currentState.loanRequest
          : currentState.balance,
    };
  }

  if (action.type === "closeAccount") {
    return {
      ...currentState,
      balance: currentState.balance === 0 ? 0 : currentState.balance,
      loan: currentState.loan !== 0 ? currentState.loan : 0,
      isActive:
        currentState.balance === 0 && currentState.loan === 0 ? false : true,
      closeAccount: currentState.isActive,
    };
  }

  if (action.type === "withdraw") {
    return {
      ...currentState,
      balance:
        currentState.balance >= 50
          ? currentState.balance - 50
          : currentState.balance,
    };
  }
};

export default function App() {
  const [
    {
      balance,
      loan,
      isActive,
      deposit,
      withdraw,
      loanRequest,
      payLoan,
      closeAccount,
    },
    dispatch,
  ] = useReducer(reduce, initialState);
  const openAccount = () => {
    return dispatch({ type: "open", payload: true });
  };

  const depositCash = () => {
    dispatch({ type: "deposit" });
  };

  const requestMyLoan = () => {
    dispatch({ type: "loan" });
  };

  const payMyLoan = () => {
    dispatch({ type: "payback" });
  };

  const termiateAccount = () => {
    dispatch({ type: "closeAccount" });
  };

  const widthdrawMoney = () => {
    dispatch({ type: "withdraw" });
  };
  return (
    <div className="App">
      <h1>useReducer Bank Account</h1>
      <p>
        Balance: {balance > 0 && "R"}
        {balance === 0 ? "X" : balance}
      </p>
      <p>
        Loan: {loan !== 0 && "R"}
        {loan === 0 ? "X" : loan}
      </p>

      <p>
        <button onClick={openAccount} disabled={isActive}>
          Open account
        </button>
      </p>
      <p>
        <button onClick={depositCash} disabled={!isActive}>
          Deposit 150
        </button>
      </p>
      <p>
        <button onClick={widthdrawMoney} disabled={!isActive}>
          Withdraw 50
        </button>
      </p>
      <p>
        <button onClick={requestMyLoan} disabled={!isActive}>
          Request a loan of 5000
        </button>
      </p>
      <p>
        <button onClick={payMyLoan} disabled={!isActive}>
          Pay loan
        </button>
      </p>
      <p>
        <button onClick={termiateAccount} disabled={!isActive}>
          Close account
        </button>
      </p>
    </div>
  );
}
